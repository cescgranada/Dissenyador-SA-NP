...
export const Phase2StructureView: React.FC<Phase2StructureProps> = ({ data, onNext, onBack }) => {
  ...
  const handleDownload = (generator: () => string, filename: string, mimeType: string) => {
    try {
      ...
      if (!content) {
        throw new Error("El contingut generat està buit.");
      }
      ...
      if (mimeType === 'application/json') {
        try {
          ...
          throw new Error("L'arxiu Colab generat no és un JSON vàlid.");
        }
      } else if (filename.endsWith('.tex')) {
        // Basic LaTeX validation: Balanced braces
        const openBraces = (content.match(/\{/g) || []).length;
        const closeBraces = (content.match(/\}/g) || []).length;
        if (openBraces !== closeBraces) {
           const proceed = window.confirm(`Atenció: S'han detectat claus {} desequilibrades al codi LaTeX (${openBraces} obertes vs ${closeBraces} tancades). Això pot causar errors de compilació. Vols descarregar igualment?`);
           if (!proceed) return;
        }
      }
      ...
      alert(`Error durant la descàrrega: ${error.message}`);
      console.error(error);
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-emerald-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2">
             <button onClick={() => handleDownload(() => getColabContent(), 'estructura_colab.ipynb', 'application/json')} className="px-3 py-1.5 bg-white border border-slate-300 hover:bg-slate-50 rounded text-xs font-bold text-slate-700 transition shadow-sm flex items-center gap-1">
               🐍 Colab
             </button>
             <button onClick={() => handleDownload(() => getLatexContent(), 'estructura.tex', 'text/plain')} className="px-3 py-1.5 bg-white border border-slate-300 hover:bg-slate-50 rounded text-xs font-bold text-slate-700 transition shadow-sm flex items-center gap-1">
               T Overleaf
             </button>
             <button onClick={() => handleDownload(() => getMarkdownContent(), 'estructura.md', 'text/markdown')} className="px-3 py-1.5 bg-white border border-slate-300 hover:bg-slate-50 rounded text-xs font-bold text-slate-700 transition shadow-sm flex items-center gap-1">
               ⬇ Word
             </button>
          </div>
        </div>
