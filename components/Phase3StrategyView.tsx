      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        <div className="p-6 bg-gradient-to-r from-purple-700 to-pink-700 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2">
             <button onClick={() => handleDownload(() => getColabContent(), 'estrategia_colab.ipynb', 'application/json')} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded text-xs font-bold border border-white/30 transition flex items-center gap-1">
               🐍 Colab
             </button>
             <button onClick={() => handleDownload(() => getLatexContent(), 'estrategia.tex', 'text/plain')} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded text-xs font-bold border border-white/30 transition flex items-center gap-1">
               T Overleaf
             </button>
             <button onClick={() => handleDownload(() => getMarkdownContent(), 'estrategia.md', 'text/markdown')} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded text-xs font-bold border border-white/30 transition flex items-center gap-1">
               ⬇ Word
             </button>
          </div>
        </div>
