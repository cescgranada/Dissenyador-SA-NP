...
`;
  ...
  // --- HELPERS FOR GLOBAL DOWNLOAD ---
  const generateMarkdownAll = () => {
    let md = "# Fitxes de Treball de l'Alumnat\n\n";
    worksheets.forEach(sheet => {
      md += getMarkdownSingle(sheet) + "---\n\n";
    });
    return md;
  };
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        <div className="p-6 bg-gradient-to-r from-sky-600 to-blue-500 text-white flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-2">
             <button onClick={() => handleDownload(() => generateMarkdownAll(), 'totes_fitxes.md', 'text/markdown')} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded text-xs font-bold border border-white/30 transition">
              Descarrega TOT (Markdown)
             </button>
          </div>
        </div>
            ...
            <div key={idx} className="p-8 bg-white print:p-0">
              <div className="flex flex-col xl:flex-row justify-between xl:items-center mb-6 border-b border-slate-100 pb-4 gap-4">
                 ...
                 <div className="flex flex-wrap gap-2">
                    <button 
                      onClick={() => handleDownload(() => getColabSingle(sheet), `fitxa_${idx}_colab.ipynb`, 'application/json')}
                    ...
                    <button 
                      onClick={() => handleDownload(() => getLatexSingle(sheet), `fitxa_${idx}.tex`, 'text/plain')}
                    ...
                    <button 
                      onClick={() => handleDownload(() => getMarkdownSingle(sheet), `fitxa_${idx}.md`, 'text/markdown')}
