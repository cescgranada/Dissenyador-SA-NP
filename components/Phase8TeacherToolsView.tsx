...
export const Phase8TeacherToolsView: React.FC<Phase8TeacherToolsProps> = ({ tools, onReset }) => {
  ...
  const getLatexContent = (tool: EvaluationTool) => {
    ...
    // We try to sanitize the content as best as possible, assuming content is largely text/markdown from AI.
    // Note: AI content can be unpredictable, strict escaping might break intended latex if AI outputs latex code.
    // Here we treat content as plain text mostly.
...
\\title{${safeText(tool.instrumentName)}}
\\date{}
...
${tool.relatedActivity ? `\\noindent\\textbf{Activitat:} ${safeText(tool.relatedActivity)}\\\\` : ''}
\\noindent\\textbf{Descripció:} ${safeText(tool.description)}

\\vspace{0.5cm}
\\hrule
\\vspace{0.5cm}

% El contingut generat es mostra en verbatim per evitar errors de compilació amb caràcters especials
\\begin{verbatim}
${tool.content}
\\end{verbatim}
            ...
            <div key={idx} className="p-8 bg-white">
              <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
                 ...
                 <div className="flex flex-wrap gap-2 w-full xl:w-auto">
                    <button 
                      onClick={() => handleDownload(() => getColabContent(tool), `eina_${idx}_colab.ipynb`, 'application/json')}
                    ...
                    <button 
                      onClick={() => handleDownload(() => getLatexContent(tool), `eina_${idx}_overleaf.tex`, 'text/plain')}
                    ...
                    <button 
                      onClick={() => handleDownload(() => getMarkdownContent(tool), `eina_${idx}_word.md`, 'text/markdown')}
